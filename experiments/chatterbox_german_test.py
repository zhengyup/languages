"""Minimal Chatterbox multilingual German TTS experiment.

Install: pip install chatterbox-tts torchaudio
Run: python experiments/chatterbox_german_test.py
Change text: edit TEST_CASES below and keep language_id="de".
...
- This mirrors the validated Mandarin experiment but targets German.
...
- Multiple German sentences make it easier to tune by sentence length.
"""
...
TEST_CASES = [
    ("german_test_trimmed.wav", "Guten Tag, wie kann ich Ihnen helfen?"),
    ("german_test_trimmed_short.wav", "Wo ist der Bahnhof?"),
    ("german_test_trimmed_medium.wav", "Hallo, ich habe eine Reservierung."),
    ("german_test_trimmed_long.wav", "Entschuldigung, ich möchte morgen früh ein Ticket nach Berlin kaufen."),
]
...
def custom_generate(model: ChatterboxMultilingualTTS, text: str, language_id: str = "de", max_new_tokens: int | None = None, cfg_weight: float = CFG_WEIGHT, exaggeration: float = EXAGGERATION) -> torch.Tensor:
...
def apply_pitch_shift(
    ...
    """
    Slowing the waveform with interpolation also lowers pitch.
    Auto compensation shifts the result back up by the amount introduced by the
    slowdown, and PITCH_SHIFT_SEMITONES lets you fine-tune further.
    """
    compensation = 0.0
...
def main() -> None:
    ...
    for output_path, text in TEST_CASES:
        ...
        raw_wav = custom_generate(model, text, language_id="de", max_new_tokens=max_new_tokens, cfg_weight=CFG_WEIGHT, exaggeration=EXAGGERATION)
