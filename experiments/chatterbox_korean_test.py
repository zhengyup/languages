"""Minimal Chatterbox multilingual Korean TTS experiment.

Install: pip install chatterbox-tts torchaudio
Run: python experiments/chatterbox_korean_test.py
Change text: edit TEST_CASES below and keep language_id="ko".
...
- This mirrors the Mandarin, German, and Spanish experiments but targets Korean.
...
- Multiple Korean sentences make it easier to tune by sentence length.
"""
...
TEST_CASES = [
    ("korean_test_trimmed.wav", "안녕하세요, 무엇을 도와드릴까요?"),
    ("korean_test_trimmed_short.wav", "역이 어디에 있어요?"),
    ("korean_test_trimmed_medium.wav", "안녕하세요, 예약이 있습니다."),
    ("korean_test_trimmed_long.wav", "실례합니다, 내일 아침에 서울행 표를 사고 싶어요."),
]
...
def custom_generate(model: ChatterboxMultilingualTTS, text: str, language_id: str = "ko", max_new_tokens: int | None = None, cfg_weight: float = CFG_WEIGHT, exaggeration: float = EXAGGERATION) -> torch.Tensor:
...
def main() -> None:
    ...
    for output_path, text in TEST_CASES:
        ...
        raw_wav = custom_generate(model, text, language_id="ko", max_new_tokens=max_new_tokens, cfg_weight=CFG_WEIGHT, exaggeration=EXAGGERATION)
