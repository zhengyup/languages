"""Minimal Chatterbox multilingual Japanese TTS experiment.

Install: pip install chatterbox-tts torchaudio
Run: python experiments/chatterbox_japanese_test.py
Change text: edit TEST_CASES below and keep language_id="ja".
...
- This mirrors the Mandarin, German, Spanish, and Korean experiments but targets Japanese.
...
- Multiple Japanese sentences make it easier to tune by sentence length.
"""
...
TEST_CASES = [
    ("japanese_test_trimmed.wav", "こんにちは、ご案内しましょうか。"),
    ("japanese_test_trimmed_short.wav", "駅はどこですか。"),
    ("japanese_test_trimmed_medium.wav", "こんにちは、予約があります。"),
    ("japanese_test_trimmed_long.wav", "すみません、明日の朝に東京行きの切符を買いたいです。"),
]
...
def custom_generate(model: ChatterboxMultilingualTTS, text: str, language_id: str = "ja", max_new_tokens: int | None = None, cfg_weight: float = CFG_WEIGHT, exaggeration: float = EXAGGERATION) -> torch.Tensor:
...
def main() -> None:
    ...
    for output_path, text in TEST_CASES:
        ...
        raw_wav = custom_generate(model, text, language_id="ja", max_new_tokens=max_new_tokens, cfg_weight=CFG_WEIGHT, exaggeration=EXAGGERATION)
